...
# --- 4. Download required scripts into ~/cerebro_scripts ---
SCRIPT_DIR="$HOME/cerebro_scripts"
mkdir -p "$SCRIPT_DIR"
echo "[*] Downloading ram_build.sh, rparu, and rpacman into $SCRIPT_DIR..."
for f in ram_build.sh rparu rpacman; do
  curl -fsSL "https://raw.githubusercontent.com/cerebro-tech/Cerebro/refs/heads/main/$f" \
    -o "$SCRIPT_DIR/$f"
...
for cfg in makepkg.conf rust.conf; do
  ...
  curl -fsSL "https://raw.githubusercontent.com/cerebro-tech/Cerebro/refs/heads/main/$cfg" \
    | sudo tee "/etc/$cfg" >/dev/null
done
